    ...
    const _onload = function() {
        ...
        const canvasframe = function() {
            ...
            for (let j = 0; j < 6.28; j += 0.3) { // j <=> theta
                const ct = Math.cos(j), st = Math.sin(j); // cosine theta, sine theta
                for (let i = 0; i < 6.28; i += 0.1) { // i <=> phi
                    const sp = Math.sin(i), cp = Math.cos(i); // cosine phi, sine phi
                    ...
                    const yp = (120 - K1 * ooz * y); // y' (it's negative here because in our output, positive y goes down but in our 3D space, positive y goes up)
